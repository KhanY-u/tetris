...
public class Board extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
	...
	// field
    private Color[][] boardColor = new Color[boardHeight][boardWidth];
    ...
    public Board() {
        ...
        pause = Image.loadImage("/pause.png");
        refresh = Image.loadImage("/refresh.png");
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int row = 0; row < boardColor.length; row++) {
            for (int col = 0; col < boardColor[row].length; col++) {
        ...
        g.drawString("SCORE", Main.WIDTH - 125, Main.HEIGHT / 2);
        g.drawString(score + "", Main.WIDTH - 125, Main.HEIGHT / 2 + 30);
        ...
        for (int row = 0; row < currentShape.getCoords().length; row++) {
            for (int col = 0; col < currentShape.getCoords()[0].length; col++) {
                if (currentShape.getCoords()[row][col] != 0) {
                    if (boardColor[currentShape.getY() + row][currentShape.getX() + col] != null) {
                        gameOver = true;
                    }
                }
            }
